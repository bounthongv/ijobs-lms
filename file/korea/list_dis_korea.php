...
$sql_max = $conn->prepare("SELECT MAX(disk_id) FROM district_korea");
$sql_max->execute();
$max_id = $sql_max->fetchColumn();
$number = 1;
$number = $max_id ? (int) $max_id + 1 : 1;
$disk_id = str_pad($number, 4, '0', STR_PAD_LEFT);
$sql_prok = $conn->prepare("SELECT * FROM province_korea");
$sql_prok->execute();
$s_prok = $sql_prok->fetchAll(PDO::FETCH_ASSOC);

$sql_agency = $conn->prepare("SELECT * FROM agency_korea ORDER BY agen_id ASC");
$sql_agency->execute();
$agency = $sql_agency->fetchAll(PDO::FETCH_ASSOC);
?>
<style>
  table th,
  table td {
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.4rem; flex-wrap:wrap; gap:10px;">
  <div>
    <h5 style="font-size:19px; font-weight:700; color:#0f172a; margin:0;">
      <i class="bi bi-buildings-fill me-2 text-primary"></i>District Korea List
    </h5>
  </div>
...
<div class="dash-card">
  <div class="dash-card-title">
    <i class="bi bi-table text-primary"></i> ລາຍຊື່ District Korea ທັງໝົດ
  ...
  <div class="filter-bar row g-3 mb-3">
    <div class="col-md-4 filter-group">
      ...
      <input type="text" name="all" id="s_all" placeholder="🔍 ຄົ້ນຫາຊື່ເມືອງ/Agency...">
    </div>
    <div class="col-md-2 filter-group">
      <label for="all">Province</label>
      <select name="" id="s_prok_id" class="form-select">
        <option value="">ເລືອກ</option>
        <?php foreach($s_prok as $pk): ?>
          <option value="<?= $pk['prok_id'] ?>"><?= $pk['prok_name'] ?></option>
        <?php endforeach ?>
      </select>
    </div>
    <div class="col-md-2 filter-group btn-mt">
      <button type="button" class="btn btn-secondary btn-sm" id="sc_disk"><i class="bi bi-search"></i> Search</button>
    </div>
  </div>
...
include_once('form/disk_add.php');
include_once('form/disk_edit.php');
