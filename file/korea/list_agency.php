...
$sql = $conn->prepare("SELECT * FROM agency_korea ORDER BY agen_id ASC");
$sql->execute();
$num = 1;
...
$sql_max = $conn->prepare("SELECT MAX(agen_id) FROM agency_korea");
$sql_max->execute();
$max_id = $sql_max->fetchColumn();
$number = 1;
$number = $max_id ? (int) $max_id + 1 : 1;
$agen_id = str_pad($number, 3, '0', STR_PAD_LEFT);
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.4rem; flex-wrap:wrap; gap:10px;">
  <div>
    <h5 style="font-size:19px; font-weight:700; color:#0f172a; margin:0;">
      <i class="bi bi-briefcase-fill me-2 text-primary"></i> Agency List
    </h5>
  </div>
...
<div class="dash-card">
  <div class="dash-card-title">
    <i class="bi bi-table text-primary"></i> ລາຍຊື່ Agency ທັງໝົດ
  ...
  <div class="filter-bar row g-3 mb-3">
    <div class="col-md-4 filter-group">
      ...
      <input type="text" name="all" id="s_all" placeholder="🔍 ຄົ້ນຫາຊື່ Agency...">
    </div>

    <div class="col-md-2 filter-group btn-mt">
      <button type="button" class="btn btn-secondary btn-sm" id="sc_agen"><i class="bi bi-search"></i> Search</button>
    </div>
  </div>
...
include_once('form/agen_add.php');
include_once('form/agen_edit.php');
